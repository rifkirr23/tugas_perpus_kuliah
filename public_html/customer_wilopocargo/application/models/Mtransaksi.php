...
class Mtransaksi extends CI_Model {
	...
	function get_transaksi() {
		...
		$this->datatables->where('transaksi.id_cust',$idcust);
		$idtransaksi="$1";
        $this->datatables->add_column('view','<a href="#" onclick="view_image('.$idtransaksi.')" alt="Update Data">Lihat</a>', 'id_transaksi');
  	...
  	function save()
  {
		...
		$kode=$this->session->userdata('id_cust');
		$kodeg= "";
    ...
    foreach($data_kurs->result() as $krs ){
      ...
      $fee_cs=$krs->fee_cs;
		...
		$data_kode= $this->get_idcust($kode);
    foreach($data_kode->result() as $koderow ){
            $email    =$koderow->email;
			$whatsapp =$koderow->whatsapp;
            $id_cust  =$koderow->id_cust;
			$nama_cust=$koderow->nama;
			$kodecust=$koderow->kode;
			$id_referal=$koderow->id_referal;
    }

		$data_kodegrup= $this->get_idgrup($kodeg);
    foreach($data_kodegrup->result() as $kodegrow ){
            $email_grup    =$kodegrow->email_cgrup;
			$whatsapp_grup =$koderow->whatsapp_cgrup;
            $id_cgrup   	 =$kodegrow->id_cgrup;
			$nama_cgrup  	 =$kodegrow->nama_cgrup;
    }
			...
			if($kode==""){
			   $status_invoice="grup";
				 ...
				 $kodeaktif = $this->input->post('kode_cgrup');
				 $namaaktif = $nama_cgrup;
			}else if($kodeg==""){
				 $status_invoice="customer";
				 $id_cgrup =0;
				 $emailaktif= $email;
				 $whatsappaktif= $whatsapp;
				//  $whatsappaktif= '089662019719';
				 $kodeaktif = $kodecust;
				 $namaaktif = $nama_cust;
			}else	if($kode=="" && $kodeg==""){
				$this->session->set_flashdata('msg','gagal_input');
				redirect(site_url('titiptransfer'));
			}
      ...
      for($ff=0;$ff<$ja;$ff++){
        ...
        foreach ($_FILES['file_bank_tujuan']['name'] as $key => $image) {
            ...
            if($_FILES['file_bank_tujuan']['name'][$key] == "")
            {
              ...
              move_uploaded_file($_FILES["file_bank_tujuan"]["tmp_name"][$key], '../office/assets/bank_tujuan/'.$_FILES["file_bank_tujuan"]["name"][$key]);
...
// 				$test['record']= $this->grup_invoiceid($last_id_invoice)->result();
// 			}

      $test['rincian']= $this->rincian_inv($last_id_invoice)->result();
      $data = $this->load->view('dashboard/invoices/pdf_invoice',$test,True);

      $mpdf = new \Mpdf\Mpdf();

      $mpdf->WriteHTML($data);
      $mpdf->Output(time()."inv.pdf" ,'F');
			$time=time()."inv.pdf";
      $content = $mpdf->Output('', 'S');
      $atch=base_url().$nama_file;
 		  ...
 		  $sendoc_cust = send_document($time,$whatsappaktif,$invoice['kode_invoice']);

 		  sendwhatsapp($pesan,"081310961108");
 		  $sendoc = send_document($time,"081310961108",$invoice['kode_invoice']);
  ...
  function update($file_lampiran1)
  {
	  ...
	  foreach ($_FILES['file_bb_rmb']['name'] as $key => $image) {
		...
		foreach($data_kode->result() as $koderow ){
					$email_customer    =$koderow->email;
					$whatsapp_customer =$koderow->whatsapp;
					$kode_customer     =$koderow->kode;
					$nama_customer     =$koderow->nama;
		}

		$data_kodegrup= $this->get_idcgrup($this->input->post('id_cgrup'));
		foreach($data_kodegrup->result() as $kodegrow ){
					$email_grup    =$kodegrow->email_cgrup;
					$whatsapp_grup =$koderow->whatsapp_cgrup;
					$kode_cgrup    =$kodegrow->kode_cgrup;
					$nama_customergrup=$kodegrow->nama_cgrup;
		}

		if($this->input->post('id_cust')==0){
			$emailaktif   = $email_grup;
			$whatsappaktif= $whatsapp_grup;
			$kodeaktif    = $kode_cgrup;
			$namaaktif    = $nama_customergrup;
	 }else if($this->input->post('id_cgrup')==0){
			$emailaktif   = $email;
