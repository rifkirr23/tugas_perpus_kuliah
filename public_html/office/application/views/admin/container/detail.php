...
if(strpos($dc->kode, 'FCL') !== false){
  $tipenya = "FCL";
}else if(strpos($dc->kode, 'LCL') !== false){
  $tipenya = "LCL";
}
?>
        ...
        <span class="pull-right" style="margin-right:1%;">
          <form method="post" action="<?php echo base_url('admin/laporan/sortgiw') ?>">
            ...
            <?php if ($dc->status == 4 ){ ?>
              <?php if($tipenya == "FCL"){ ?>
                <span class="pull-right" style="margin-right:5px;">
                  <a onclick="return confirm(`Container Sudah Sampai?`);" href="<?php echo site_url('admin/container/arrived_cont/'.$this->uri->segment(4)); ?>" class="form-control btn btn-primary olay"><i class="fa fa-book"> Arrived </i></a>
                </span>
              <?php } ?>
            <?php } ?>
            <span class="pull-right" style="margin-right:5px;">
