...
class Mbank extends CI_Model {
	...
	function get_bank() {
        $this->datatables->select('master_bank.id_bank,master_bank.nama_bank,master_bank.nomor_rekening_bank,master_bank.atas_nama_bank,master_bank.saldo_bank,master_bank.edit_saldo,
																		sum(transaksi_bank.nominal_transaksi_bank) as saldo_masuk_bank');
        $this->datatables->from('transaksi_bank');
				$this->datatables->join('master_bank', 'transaksi_bank.id_bank=master_bank.id_bank');
				$this->datatables->group_by('master_bank.id_bank');
        ...
        <a href="'.site_url().'admin/bank/transaksi/$1" class="btn btn-info btn-xs"> <i class="fa fa-ellipsis-h"></i></a>
				<a href="'.site_url().'admin/bank/saldo_harian/$1" class="btn btn-primary btn-xs"> <i class="fa fa-calendar-check-o"></i></a>
        </center>',
         'id_bank,nama_bank,nomor_rekening_bank,atas_nama_bank,saldo_bank,edit_saldo');
        return $this->datatables->generate();
  }

	//Proses get data saldo
	function get_saldo_harian($id) {
        $this->datatables->select('master_bank.id_bank,master_bank.nama_bank,saldo_harian.jumlah,saldo_harian.keterangan
																	,saldo_harian.tanggal_saldo,saldo_harian.id_saldo_harian');
        $this->datatables->from('saldo_harian');
				$this->datatables->join('master_bank', 'saldo_harian.id_bank=master_bank.id_bank');
				$this->datatables->where('saldo_harian.id_bank',$id);
				$this->datatables->group_by("tanggal_saldo");
        // $this->db->order_by('saldo_harian.id_saldo_harian','desc');
				$this->datatables->add_column('view', '<center><a href="javascript:void(0);" class="edit_record btn btn-info btn-xs"
				 															 data-id_saldo_harian="$1" data-keterangan="$2" data-id_bank="$3"> <i class="fa fa-edit"></i></a> </center>',
         'id_saldo_harian,keterangan,id_bank');
        return $this->datatables->generate();
  }
