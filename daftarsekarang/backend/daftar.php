<?php
  ...
  while($data = $querysales->fetch_array()){
    ...
    if($data['id_sales'] == Null || $data['id_sales'] == 10){
      $id_sales = 5;
    }else if($data['id_sales'] == 5){
      $id_sales = 9;
    }else if($data['id_sales'] == 9){
      $id_sales = 10;
    }
  }
  // die();
...
if($response->success){
    $input = $mysqli -> query("INSERT INTO leads VALUES (NULL, '$jenis_barang', '$nama', '$whatsapp', '$email', '$sp_cargo', '$sp_supplier', '$dihub' ,'$tgl','$id_sales')")
      or die(mysqli_error());
      if($input == true){
      ...
      header("location:https://wilopocargo.com/terima-kasih/");
}else{
        header("location:../index.php?message=recaptcha#contactus");
}
 ?>
