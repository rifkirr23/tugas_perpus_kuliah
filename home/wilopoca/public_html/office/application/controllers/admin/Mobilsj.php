...
class Mobilsj extends CI_Controller {
  ...
  function index(){
		...
		$data['jumlah_today'] = $this->db->where('tanggal_kirim',date('Y-m-d'))->get('sj_wc')->num_rows();
		$data['jumlah_kemarin'] = $this->db->where('tanggal_kirim',$kemarintgl)->get('sj_wc')->num_rows();
		$data['jumlah_cbm'] = $this->db->select('sum(volume*jumlah) as total')
															->from('invoice_product')
															->join('giw', 'invoice_product.id_giw=giw.id','left')
															->join('sj_wc', 'sj_wc.id_sj=invoice_product.id_sj_wc','left')
															->where('sj_wc.status',2)
															->where('sj_wc.tanggal_kirim',date('Y-m-d'))
															->get()->row();
